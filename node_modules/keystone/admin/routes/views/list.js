...
exports = module.exports = function(req, res) {
	...
	var viewLocals = {
		validationErrors: {},
		showCreateForm: _.has(req.query, 'Nuevo')
	};
	...
	_.each(filters, function(filter, path) {
		cleanFilters[path] = _.omit(filter, 'Campo');
	});
	...
	if (sort.by) {
		...
		if (sort.field) {
			...
			if (sort.field.type === 'Nombre') {
				sort.by = sort.by + '.first ' + sort.by + '.Apellido';
			}
	...
	var checkCSRF = function() {
		...
		if (!pass) {
			...
			req.flash('error', 'Ha ocurrido un problema, intentélo de nuevo.');
		}
		return pass;
	};
	...
	if ('update' in req.query) {
		...
		(function() {
			...
			req.list.updateAll(data, function(err) {
				if (err) {
					...
					console.log(err);
					req.flash('error', 'Error al actualizar ' + req.list.plural + ' (logged to console)');
				} else {
					req.flash('success', 'All ' + req.list.plural + ' Cambios guardados.');
				}
		...
		if (req.query['delete'] === req.user.id) { //eslint-disable-line dot-notation
			req.flash('error', 'No es posible eliminar ' + req.list.singular + '.');
			return renderView();
		}
		...
		req.list.model.findById(req.query['delete']).exec(function (err, item) { //eslint-disable-line dot-notation
			...
			item.remove(function (err) {
				if (err) {
					...
					console.log(err);
					req.flash('error', 'Error eliminando ' + req.list.singular + ': ' + err.message);
				} else {
					req.flash('success', req.list.singular + ' Eliminado.');
				}
				res.redirect('/keystone/' + req.list.path);
			});
		});
		...
		item.save(function(err) {
			...
			if (err) {
				...
				console.log(err);
				req.flash('error', 'Ha ocurrido un error ' + req.list.singular + '.');
				renderView();
			} else {
				req.flash('success', 'Nuevo ' + req.list.singular + ' ' + req.list.getDocumentName(item) + ' created.');
