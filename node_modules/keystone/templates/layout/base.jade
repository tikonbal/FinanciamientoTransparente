	...
	body(id='page-' + page)
		#wrap
			nav(role='navigation')#header.navbar.navbar-default.navbar-static-top: .container
				...
				.navbar-backtobrand-message Regresar a #{brand}
				.navbar-header
				...
				.collapse.navbar-collapse.navbar-headernav-collapse
					if signout
						ul.nav.navbar-nav.navbar-right
							li.divider
							li: a(href=signout).signout Cerrar Sesión
					ul.nav.navbar-nav
		...
		#footer: .container
			p 	#{brand} #{appversion}
				| Powered by <a href="http://tikonbal.com" target="_blank">KeystoneJS</a> version #{version}.
				if User && user
					|  Signed in as
